<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="d-flex align-items-center justify-content-between mt-4">
            <a class="text-decoration-underline small text-white-50" href="{{ route('login') }}">{{ __('Ya estás registrado?') }}</a>
            <button type="submit" class="btn btn-danger px-4 fw-semibold">{{ __('Registrarse') }}</button>
        </div>
    </form>
</x-guest-layout>