<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="form-check mb-3">
            ...
            <label class="form-check-label text-white-50" for="remember_me">{{ __('Recordarme') }}</label>
        </div>

        <div class="d-flex align-items-center justify-content-between">
            @if (Route::has('password.request'))
                <a class="text-decoration-underline small text-white-50" href="{{ route('password.request') }}">
                    {{ __('Has olvidado la contraseña?') }}
                </a>
            @endif
            <button type="submit" class="btn btn-danger px-4 fw-semibold">Iniciar Sesión</button>
        </div>
    </form>
</x-guest-layout>